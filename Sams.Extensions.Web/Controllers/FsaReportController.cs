...
namespace Sams.Extensions.Web.Controllers
{
    ...
    public class FsaReportController : Controller
    {
        ...
        public ActionResult Fsa(string ParamDetails,string AuditDate)
        {
            ...
            ViewBag.Year = DateTime.Now.Year + "-" + (DateTime.Now.Year + 1).ToString();
            //List<FsaReportDetails> aud = new List<FsaReportDetails>();
            SqlCommand cmd = new SqlCommand("select * from APS_FSA_Master where LocationCode='"+vm.CurrentClient+"'", con);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
            if (dt.Rows.Count > 0)
            {
                ViewBag.Date =Convert.ToDateTime(dt.Rows[0]["Date"]).ToString("dd MMMM yyyy");
            }
            //foreach (DataRow dr in ds.Tables[3].Rows)
            //{
            //    aud.Add(new FsaReportDetails
            //    {
            //        //aud.AuditDate = Convert.ToDateTime(dr["Date"])
            //    });
            //}
            return View(vm);
        }
